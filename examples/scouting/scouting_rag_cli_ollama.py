...
def log_retrieval(
    ...
    # Conteo de origen en el set final
...
def print_retrieval_summary(
    ...
    total_tokens = sum(estimate_tokens(d.content or "") for d in final_docs)
    ctx_info = (
        f"  [dim]·[/dim]  [dim]~{total_tokens} tok ({total_tokens * 100 // num_ctx}% de num_ctx)[/dim]"
        if num_ctx else
...
def run_interactive_loop(
    ...
    while True:
        ...
        print_retrieval_summary(semantic_docs, keyword_docs, final_docs, num_ctx=num_ctx)
...
def log_session_start(cfg: Config) -> None:
    ...
    log(f"║  num_ctx        {cfg.num_ctx:<41}║")
    log(f"║  num_predict    {cfg.num_predict:<41}║")
...
if __name__ == "__main__":
    ...
    try:
        run_interactive_loop(retrieval_pipeline, generation_pipeline, num_ctx=cfg.num_ctx)
    finally:
