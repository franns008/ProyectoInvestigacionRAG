...
[`src/pipeline/indexing/run_indexing.py`](../src/pipeline/indexing/run_indexing.py),
método `Indexer.run()`:

```python
splitter = DocumentSplitter(
    split_by="word", split_length=SPLIT_LENGTH, split_overlap=SPLIT_OVERLAP,  # 200 / 20
)
splittable_docs = splitter.run(documents=splittable_docs)["documents"]
```

> La indexación **se desacopló** del pipeline de inferencia: antes vivía en
> `pipeline_ciberseguridad.py::_index_new_documents()` y los parámetros de chunking
> estaban en los `Valves`. Hoy son constantes de módulo (`SPLIT_LENGTH`,
> `SPLIT_OVERLAP`) del script de indexación, que corre aparte del server.
...
### 7.2 Cómo correrlo (container efímero)

Se usa un container **efímero** que saltea el server de Open WebUI y corre Python directo:
mantiene el experimento aislado del runtime y evita cargar el server para nada. En
**Fedora/SELinux** los bind-mounts requieren `:z`; **no** se monta el volumen
`marker_cache` (el reranker ya vive en la imagen). Desde `infrastructure/`:

> **Nota:** la razón original de evitar `docker compose exec pipelines` era que el server
> **auto-indexaba** el corpus de producción al arrancar (horas de embeddings por los ~76k
> CVE de NVD). Eso **ya no pasa**: tras desacoplar la indexación, `on_startup()` sólo
> cuenta los documentos del store. El container efímero se mantiene igual por aislamiento.
...
| 3 | Si gana **D**: cambiar el pipeline para pasar **markdown crudo** al splitter markdown-aware (evitar `MarkdownToDocument` para PDFs) | Aprovecha la estructura de sección | Toca `Indexer.load_documents()` |
